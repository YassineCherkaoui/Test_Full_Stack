...
const poker = require('./App')
const { Winner } = require('./App')
...
const hearts = ["A♡", "10♡", "J♡", "Q♡", "K♡"]
const spades = ["A♤", "10♤", "J♤", "Q♤", "K♤"]
...
io.on('connection', (socket) => {
    socket.on('join', ({ name, room }, callback) => {
        ...
        socket.emit('message', {user: '', text: `Bonjour ${user.name}! en Casino à Las Vegas,`})
        socket.broadcast.to(user.room).emit('message', {user: '', text: `${user.name} a rejoint.`})
